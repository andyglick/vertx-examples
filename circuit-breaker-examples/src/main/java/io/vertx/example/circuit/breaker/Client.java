  ...
  public void start() {
    ...
    CircuitBreaker breaker =
      CircuitBreaker.create("my-circuit-breaker", vertx, options)
        .openHandler(v -> System.out.println("Circuit opened")).closeHandler(v -> System.out.println("Circuit closed"));

    breaker.executeWithFallback(promise -> vertx.createHttpClient().request(HttpMethod.GET, 8080, "localhost", "/").compose(req -> {
